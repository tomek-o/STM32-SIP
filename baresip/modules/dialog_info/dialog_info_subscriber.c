	...
	switch(evt) {
		case XML_EVENT_START_NODE:
			if (!ctx->in_dialog_info) {
				if (!str_casecmp(node->tag, C2SX("dialog-info"))) {
			...
			if (!ctx->in_dialog_info)
				break;
			if (!ctx->in_dialog) {
				if (!str_casecmp(node->tag, C2SX("dialog"))) {
					SXML_CHAR* val = NULL;
					ctx->in_dialog = true;
					// fill defaults
			...
			if (!ctx->in_dialog)
				break;
			if (!ctx->in_remote) {
				if (!str_casecmp(node->tag, C2SX("remote")))
					ctx->in_remote = true;
			}

			if (ctx->in_remote && !ctx->in_identity) {
				if (!str_casecmp(node->tag, C2SX("identity"))) {
					SXML_CHAR* val = NULL;
			...
			if (!ctx->in_state) {
				if (!str_casecmp(node->tag, C2SX("state"))) {
					ctx->in_state = true;
				}
			}
