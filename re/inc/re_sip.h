...
enum sip_hdrid {
	...
	SIP_HDR_ACCESS_URL                    = 2962,	///< non-standard, example: Access-URL: <http://server.polycom.com/content23456.xhtml>;expires=60;mode=passive
...
enum {
	...
	SIP_T4 = 5000,
#else
    /* shorter times for low-memory device */
	SIP_T1 =  250,
	SIP_T2 = 2000,
	SIP_T4 = 2500,
#endif
...
struct sip_call_info {
	int32_t answer_after;
};

/** SIP Alert-Info header
	e.g. Alert-Info: info=<Bellcore-dr1>
*/
struct sip_alert_info {
	struct pl info;
};

enum sip_access_url_mode {
	SIP_ACCESS_URL_MODE_UNKNOWN = 0,
	SIP_ACCESS_URL_MODE_PASSIVE,
	SIP_ACCESS_URL_MODE_ACTIVE
};

/** SIP Access-URL header
	e.g. Access-URL: <http://server.polycom.com/content23456.xhtml>;expires=60;mode=passive
*/
struct sip_access_url {
	struct pl url;
	enum sip_access_url_mode mode;
	int32_t expires;
};
