...
static void tcp_recv_handler(int flags, void *arg)
{
 ...
 read:
	...
	if (0 == n) {
		...
		/** \note This conn_close() call was added after receiving report with above
			log printed repeatably and 100% CPU usage after call from Polycom phone
			using TCP transport, when TCP connection was closing after the call.
...
#endif
#if LWIP_SOCKET
		DEBUG_WARNING("listen: getaddrinfo: %s:%s error=%d\n",
			      addr, serv, error);
#else
...
#endif
#if LWIP_SOCKET
		DEBUG_WARNING("sock_bind: getaddrinfo: %s:%s error=%d\n",
			      addr, serv, error);
#else
		   ...
		   tcp_recv_h *rh, tcp_close_h *ch, void *arg)
{
	...
	if (error) {
#if LWIP_SOCKET
		DEBUG_WARNING("connect: getaddrinfo(): (%d)\n", error);
#else
...
int tcp_conn_bind(struct tcp_conn *tc, const struct sa *local)
{
	...
	if (error) {
#if LWIP_SOCKET
		DEBUG_WARNING("conn_bind: getaddrinfo(): (%d)\n", error);
#else
