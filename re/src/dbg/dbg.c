...
print_h on_print_fn = NULL;
void print_handler_set(print_h on_print)
{
	on_print_fn = on_print;
}
...
static void dbg_vprintf(int level, const char *fmt, va_list ap)
{
	...
	if (dbg.flags & DBG_ANSI) {
		...
		case DBG_WARNING:
			//(void)re_fprintf(err_stream, "\x1b[31m"); /* Red */
			break;

		case DBG_NOTICE:
			//(void)re_fprintf(err_stream, "\x1b[33m"); /* Yellow */
			break;

		case DBG_INFO:
			//(void)re_fprintf(err_stream, "\x1b[32m"); /* Green */
			break;
	...
	if (dbg.flags & DBG_TIME) {
		...
		//(void)re_vprintf("[%09llu] ", ticks - dbg.tick);
	}

	(void)re_vprintf(fmt, ap);

//	if (dbg.flags & DBG_ANSI && level < DBG_DEBUG)
//		(void)re_fprintf(err_stream, "\x1b[;m");
