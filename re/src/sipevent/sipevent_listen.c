...
#define UNSOLICITED_NOTIFY_H_MAX 2
static int unsolicited_notify_h_cnt = 0;
struct {
	char event[96];
	sipevent_unsolicited_notify_h *handler;
} unsolicited_notify_handlers[UNSOLICITED_NOTIFY_H_MAX];

/** \brief Handle unsolicited NOTIFY (no Subscription-State Header
*/
static int unsolicited_notify_handler(struct sipevent_sock *sock,
			   const struct sip_msg *msg, struct sipevent_event* event) {
	int i;
	struct sip *sip = sock->sip;

	for (i=0; i<unsolicited_notify_h_cnt; i++) {
		if (pl_strcasecmp(&event->event, unsolicited_notify_handlers[i].event) == 0) {
			return unsolicited_notify_handlers[i].handler(sock, msg);
		}
	}
...
int sipevent_reset_unsolicited_handlers(void) {
	unsolicited_notify_h_cnt = 0;
	return 0;
}

int sipevent_register_unsolicited_handler(const char *event, sipevent_unsolicited_notify_h *notifyh) {
	if (strlen(event) >= sizeof(unsolicited_notify_handlers[0].event)) {
		return -2;
	}
	if (unsolicited_notify_h_cnt < sizeof(unsolicited_notify_handlers)/sizeof(unsolicited_notify_handlers[0])) {
		unsolicited_notify_handlers[unsolicited_notify_h_cnt].handler = notifyh;
		strcpy(unsolicited_notify_handlers[unsolicited_notify_h_cnt].event, event);
		unsolicited_notify_h_cnt++;
		return 0;
	}
	return -1;
}
