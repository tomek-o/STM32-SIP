...
#ifndef RELEASE
#define MEM_DEBUG 0  /**< Enable memory debugging */
#endif
...
void *mem_realloc(void *data, size_t size)
{
	...
	m2 = pvPortRealloc(m, sizeof(*m2) + size);
...
static bool debug_handler(struct le *le, void *arg)
{
	...
	(void)re_printf("  %p: nrefs=%-2u", p, m->nrefs);

	(void)re_printf(" size=%-7u", m->size);
